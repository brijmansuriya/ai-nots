...
export default function CreateModel({ platform }: CreateModelProps) {
    ...
    const submit = (e: React.FormEvent) => {
        e.preventDefault();
        post(`/admin/platforms/${platform.id}/models`);
    };

    return (
        <AdminLayout breadcrumbs={breadcrumbs}>
            <Head title={`Add Model to ${platform.name}`} />
            <div className="flex items-center justify-between mb-4 px-4 pt-4">
                <h1 className="text-lg font-semibold text-foreground">Add Model to {platform.name}</h1>
            </div>
            ...
            <form onSubmit={submit} className="px-4 py-4 space-y-6 max-w-2xl bg-card border rounded-lg m-4">
                ...
                <div className="flex gap-2 pt-4 border-t">
                    <Button type="submit" disabled={processing}>Add Model</Button>
                    <Button
                        type="button"
                        variant="outline"
